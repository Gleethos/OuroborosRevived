...
public class dialog_script_old : MonoBehaviour {
	...
	void Update () {
		...
		if(waitingForEnter==true){
			if(Input.GetKeyDown (KeyCode.Return))
				...
				if(rawDialog == ""){//Debug.Log ("raw dialog empty!");}else{//Debug.Log ("raw dialog NOT empty");
				}
				if(paused){//Debug.Log ("paused true");}else{//Debug.Log ("paused false");
					}
			}
		}
		...
		if(waitingForAnswer==true){
			...
			{//Debug.Log ("Player has given an answer!");
				...
				if (Input.GetKeyDown (KeyCode.Keypad1)) {AnswerID = 1;}
				if (Input.GetKeyDown (KeyCode.Keypad2)) {AnswerID = 2;}
				if (Input.GetKeyDown (KeyCode.Keypad3)) {AnswerID = 3;}
				if (Input.GetKeyDown (KeyCode.Keypad4)) {AnswerID = 4;}
				if (Input.GetKeyDown (KeyCode.Keypad5)) {AnswerID = 5;}
				if (Input.GetKeyDown (KeyCode.Keypad6)) {AnswerID = 6;}
				if (Input.GetKeyDown (KeyCode.Keypad7)) {AnswerID = 7;}
				if (Input.GetKeyDown (KeyCode.Keypad8)) {AnswerID = 8;}
				if (Input.GetKeyDown (KeyCode.Keypad9)) {AnswerID = 9;}
			}
		}
		...
		if (rawDialog != "" && paused == false && waitingForEnter == false && waitingForAnswer==false) {
			...
			for (int i = dialogIndex; i < rawDialog.Length; i++)//looping through the raw dialog content!
			{
				...
				if(jumpToString!="")
				   {if(i==rawDialog.Length-1){i=0; //Debug.Log("Interpreter is restarting loop now! (jumpTo message not found!)");
					}}
			}
			...
			dialogtxt = "";
		if(paused==false && rawDialog=="" && waitingForEnter==false && waitingForAnswer==false){end = true;}//&& waitingForAnswer==false
	}
	...
	private void executeMetaCommand(string metaCommand)
	{
		...
		//===================================================
			...
			dialogOutput = value;
			Debug.Log ("This is the returnMessage: " + dialogOutput);
	...
	private void loadAndPlaySoundFile(string filename, string subdirectoryName)
	{
		...
		if(clip==null){//Debug.Log ("Audio file cold not be loaded. Audio clip empty... :/");
		}
		else{//Debug.Log("Loading successful.");
			}
